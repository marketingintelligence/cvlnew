...
class ReviewsController extends RController
{
    ...
    public function actionList() {
        ...
        if (isset($_GET['Reviews'])) {
            ...
            Yii::app()->user->setState('_filter_Reviews', $_GET['Reviews']);
        } else
            if (Yii::app()->user->hasState('_filter_Reviews')) {
                $model->attributes = Yii::app()->user->getState('_filter_Reviews');
            }
    ...
    public function actionUpdate($id) {
        ...
        $this->performAjaxValidation($model);
        $page = (int)Yii::app()->request->getParam('Reviews_page');
    ...
    public function actionDelete($id) {
       //$id = Yii::app()->request->getParam('id');
			...
			$model = Reviews::model()->find("id = '".$id."'");
			$model->delete();
            if (!isset($_GET['ajax'])) {
    ...
    public function beforeAction($action) {
        if ($_GET['ajax'] === 'state')
            if (count($_GET['ReviewsCheck']) > 0) {
                foreach ($_GET['ReviewsCheck'] as $column => $value) {
                    foreach ($value as $id => $val) {
                        Reviews::model()->updateByPk((int)$id, array($column => (int)$val));
                    }
                }
                Yii::app()->end();
            }
