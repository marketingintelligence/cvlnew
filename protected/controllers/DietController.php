<?php
    class DietController extends Controller {
        ...
        public function actionIndex() {
            ...
            $pages = new CPagination(Diet::model()->count($criteria));
            $pages->pageSize = 19;
            $pages->applyLimit($criteria);
            $model = Diet::model()->findAll($criteria);

            $this->pageTitle = "Диета";
            $this->render('index', array("model" => $model, "pages" => $pages));
        }

        public function actionCategory($url) {
            $category_id = Dietcategory::model()->find("url_text = '".$url."'");
            $criteria = new CDbCriteria();
        ...
        public function actionShow($url) {
            ...
            $model = Diet::model()->find($criteria);

            $criteria = new CDbCriteria();
            $criteria->condition = "id != '".$model->id."' AND status_int = '1'";
            $criteria->order = "rand()";
            $criteria->limit = 3;
            $poh = Diet::model()->findAll($criteria);
