...
<div class="page-navi">
    ...
    if ($this->page > 1):?>
    <a href="<?=$left?>#top"><span class="link"> ← Назад <?=$array[Yii::app()->session->itemAt('language')]['pred']?></span></a>
    <? else: $left = '#'; ?>
     <span class="link"> ← Назад <?=$array[Yii::app()->session->itemAt('language')]['pred']?></span>
    <? endif;?>
        ...
        if($this->page<5 && $this->pages<5){
          ...
          if ($i == $this->page){ 
			  ...
			  if ($i != 1) {
				...
				$bodytag = str_replace("?page=1", "", $this->controller->createUrl('', $get));
       ...
       <a href="<?=$right?>#top"><span class="link"><?=$array[Yii::app()->session->itemAt('language')]['sled']?> Вперед  →</span></a>
       <? else: $right = '#'; ?>
       <span class="link"><?=$array[Yii::app()->session->itemAt('language')]['sled']?> Вперед  →</span>
       <? endif;?>
